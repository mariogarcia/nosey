...
class GPathResultFinderExtensionSpec extends Specification {
    ...
    def 'Looking for a group of nodes within a XML'() {
        given: 'A xml with book references'
            def response = parseBooksXml()
        when: 'Looking for a specific set of books'
            def results = response.findAllBookWhere { book->
                book.author.text() == 'Miguel De Cervantes'
            }
    ...
    def 'Dynamic finder ala Gorm'() {
        given: 'A xml with book references'
            def response = parseBooksXml()
        when: 'Looking for a specific set of books'
            def results = response.findAllBookByAuthor('Miguel de Cervantes')
        then: 'We should be able to find them'
            with(results) {
                size() == 2
                first().title.text() == 'Don Xijote'
